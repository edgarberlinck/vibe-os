...
- [~] make `startx` survive input-service restart without direct-driver fallback
  : `startx` no longer wedges on the first lost `inputsvc` reply: the desktop now keeps its interactive keyboard/mouse path on the kernel-owned unified queue, still subscribes to `inputsvc` lifecycle/reset events, and preserves queued session state/reset semantics while the service transport continues to mature; the remaining gap is proving the full `kill input` -> recover -> launch-app path reliably in automated smoke and on real USB hardware, then moving steady-state publication ownership fully into `inputsvc`
- [~] desktop-launched `INPUT_EVENT` consumers now keep interactivity on the kernel-owned queue while still reacting to `inputsvc` restart/degrade events
  : this avoids the current request/reply hang window in the desktop path and keeps `startx` usable during migration, but it is still a compatibility bridge rather than the final `inputsvc`-owned steady-state publisher model
